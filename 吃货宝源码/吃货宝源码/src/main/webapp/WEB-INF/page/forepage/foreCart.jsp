                    ...
                    <div class="cart-table table-responsive mb-40">
                        ...
                        <c:forEach items="${ois}" var="oi">
                            <tr>
                                ...
                                <c:if test="${cst.status==0}"><td class="pro-price"><span>售价：${oi.product.price}</span></td></c:if>
                                <c:if test="${cst.status==1}"><td class="pro-price"><span>售价：${oi.product.price*0.8}</span></td></c:if>
                                <td class="pro-quantity"><div class="pro-qty"><input type="text" id="oiNumber${oi.id}" value="${oi.number}"></div></td>
                                <c:if test="${cst.status==0}">
                                    <td class="pro-subtotal"><span id="xiaoji1">售价：${oi.number*oi.product.price}</span></td>
                                </c:if>
                                <c:if test="${cst.status==1}">
                                    <td class="pro-subtotal"><span id="xiaoji08">售价：${oi.number*oi.product.price*0.8}</span></td>
                                </c:if>
